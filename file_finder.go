...
func (f *fileFinder) Find(d string, includedPattern, excludedPattern *regexp.Regexp, noGit bool) ([]string, error) {
	fs, err := f.findFiles(d, noGit)
	if err != nil {
		return nil, err
	}
...
func (f *fileFinder) findFiles(d string, noGit bool) ([]string, error) {
	if !noGit {
